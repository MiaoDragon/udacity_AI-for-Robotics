...
class robot:
    ...
    def move(self, motion): # Do not change the name of this function
        ...
        if abs(beta) < 0.001:
            # same as moving in a straight line
            x = (self.x + d * cos(theta)) % world_size
            y = (self.y + d * sin(theta)) % world_size
            ...
            theta = (theta + beta) % (2*pi)
            x = (cx + sin(theta)*R) % world_size
            y = (cy - cos(theta)*R) % world_size
        result = robot(L)
    ...
    # sense:
    #
    def sense(self, error=0.): #do not change the name of this function
        Z = []
